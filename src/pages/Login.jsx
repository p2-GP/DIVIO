...
export default function LoginPage() {
	...
	const { loading, setLoading, setUser, user } = useContext(dataContext);
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			onValue(res, (snapshot) => {
				...
				const index = Object.entries(datas).find((el) => el[1][0].email === userInput.email && el[1][0].password === userInput.password);

				if (index) {
					setUser({ id: index[0], ...index[1][0] });
					localStorage.setItem("userid", index[0]);
					successLogin("Login Success");
					navigate("/dashboard");
				} else {
					console.log("gada: ", index);
	...
	return (
		<div>
			<div className="w-full max-h-screen overflow-hidden h-screen p-4 flex justify-center items-center">
				<main className="w-full md:w-5/6 xl:w-4/6 h-9/10 flex items-center bg-gray-100 rounded-lg font-mono">
					<div className="w-full sm:w-1/2 md:w-1/3 p-10 grid grid-cols-1 space-y-4">
						...
						<form onSubmit={handleSubmit}>
							<div className="space-y-2">
								<input className="w-full rounded-md" type="email" name="email" value={userInput.email} onChange={handleChange} placeholder="email" />
								<input className="w-full rounded-md" type="password" name="password" value={userInput.password} onChange={handleChange} placeholder="password" />
								<button className="w-full focus:outline-none text-white text-sm py-2.5 px-5 rounded-md bg-purple-500 hover:bg-purple-600 hover:shadow-lg">Login</button>
							</div>
							<div className="text-center mt-3 mb-3">OR</div>
							<button className="w-full focus:outline-none text-white text-sm py-2.5 px-5 rounded-md bg-purple-500 hover:bg-purple-600 hover:shadow-lg">
								<Link to="/live">Join</Link>
							</button>
						</form>
						<Link to="/register" className="flex justify-around">
					...
					<div className="hidden sm:inline sm:w-1/2 md:w-2/3 p-10 md:p-20">
						...
						>
							...
							<rect x="583.16767" y="309.79837" width="44.97276" height="24.98487" fill="#8b5cf6" />
